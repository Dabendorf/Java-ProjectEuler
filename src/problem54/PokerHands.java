...
import problem54.Card.Farbe;

public class PokerHands {
	
	public boolean sortierungFarbe = true;/**true=Zahl, false=Farbe**/
	private static PokerHands ph;
	private int siegwert1;
	private int siegwert2;
	private int sieg1 = 0;
	private int sieg2 = 0;
	private int keinsieg = 0;
	...
	private void royalflush(ArrayList<Card> cards,int spieler) {
		...
		if(erfuellt) {
			...
			straightflush(cards,spieler);
		}
	}
	
	private void straightflush(ArrayList<Card> cards,int spieler) {
		Collections.sort(cards);
		boolean erfuellt = true;
		for(int i=0;i<4;i++) {
			if(cards.get(i).getFarbe().equals(cards.get(i+1).getFarbe())) {
				if(cards.get(i).getZahl()!=cards.get(i+1).getZahl()+1) {
					erfuellt = false;
					break;
				}
			} else {
		...
		if(erfuellt) {
			...
			fourofakind(cards,spieler);
		}
	}
	
	private void fourofakind(ArrayList<Card> cards,int spieler) {
		boolean erfuellt = false;
		for(int i=0;i<5;i++) {
			int zahlTemp = cards.get(i).getZahl();
			int counter = 0;
			for(int j=0;j<5;j++) {
				if(zahlTemp==cards.get(j).getZahl()) {
					counter++;
				}
			}
			if(counter==4) {
		...
		if(erfuellt) {
			...
			fullhouse(cards,spieler);
		}
	}
	
	private void fullhouse(ArrayList<Card> cards,int spieler) {
		Collections.sort(cards);
		boolean erfuellt = true;
		if(cards.get(0).getZahl()==cards.get(1).getZahl() && cards.get(1).getZahl()==cards.get(2).getZahl()) {
			if(cards.get(3).getZahl()!=cards.get(4).getZahl()) {
				erfuellt = false;
			}
		} else if(cards.get(0).getZahl()==cards.get(1).getZahl()) {
			if(!(cards.get(2).getZahl()==cards.get(3).getZahl() && cards.get(3).getZahl()==cards.get(4).getZahl())) {
				erfuellt = false;
			}
		} else {
			erfuellt = false;
	...
	private void flush(ArrayList<Card> cards,int spieler) {
		Farbe f = cards.get(0).getFarbe();
		boolean erfuellt = true;
		for(int i=1;i<5;i++) {
			if(!cards.get(i).getFarbe().equals(f)) {
				erfuellt = false;
			}
		}
		if(erfuellt) {
			...
			straight(cards,spieler);
		}
	}
	
	private void straight(ArrayList<Card> cards,int spieler) {
		Collections.sort(cards);
		boolean erfuellt = true;
		for(int i=0;i<4;i++) {
			if(cards.get(i).getZahl()!=cards.get(i+1).getZahl()+1) {
		...
		if(erfuellt) {
			...
			threeofakind(cards,spieler);
		}
	}
	
	private void threeofakind(ArrayList<Card> cards,int spieler) {
		boolean erfuellt = false;
		for(int i=0;i<5;i++) {
			int zahlTemp = cards.get(i).getZahl();
			int counter = 0;
		...
		if(erfuellt) {
			...
			twopairs(cards,spieler);
		}
	}
	
	private void twopairs(ArrayList<Card> cards,int spieler) {
		boolean erfuellt = false;
		...
		n:for(int a=0;a<5;a++) {
			for(int b=0;b<5;b++) {
				if(cards.get(a).getZahl()==cards.get(b).getZahl() && a!=b) {
					cardsTemp.remove(a);
					if(a<b) {
						cardsTemp.remove(b-1);
					} else {
						cardsTemp.remove(b);
					}
					break n;
		...
		if(cardsTemp.size()==3) {
			for(int a=0;a<3;a++) {
				for(int b=0;b<3;b++) {
					if(cardsTemp.get(a).getZahl()==cardsTemp.get(b).getZahl() && a!=b) {
						erfuellt = true;
					}
				}
			}
		...
		if(erfuellt) {
			...
			onepair(cards,spieler);
		}
	}
	
	private void onepair(ArrayList<Card> cards,int spieler) {
		boolean erfuellt = false;
		for(int a=0;a<5;a++) {
			for(int b=0;b<5;b++) {
				if(cards.get(a).getZahl()==cards.get(b).getZahl() && a!=b) {
	...
	private void hoeherStraightFlush(ArrayList<Card> cards1, ArrayList<Card> cards2) {
		Collections.sort(cards1);
		Collections.sort(cards2);
		if(cards1.get(4).getZahl()>cards2.get(4).getZahl()) {
			sieg1++;
		} else if(cards1.get(4).getZahl()<cards2.get(4).getZahl()) {
			sieg2++;
		}
	}
	
	private void hoeherFourOfAKind(ArrayList<Card> cards1, ArrayList<Card> cards2) {
		int hoch1 = 0;
		int hoch2 = 0;
		for(int i=0;i<5;i++) {
			int zahlTemp = cards1.get(i).getZahl();
			int counter = 0;
			for(int j=0;j<5;j++) {
				if(zahlTemp==cards1.get(j).getZahl()) {
					counter++;
				}
			}
			if(counter==4) {
		...
		for(int i=0;i<5;i++) {
			int zahlTemp = cards2.get(i).getZahl();
			int counter = 0;
			for(int j=0;j<5;j++) {
				if(zahlTemp==cards2.get(j).getZahl()) {
					counter++;
				}
			}
			if(counter==4) {
		...
		if(hoch1>hoch2) {
			...
			hoeherHighCard(cards1,cards2);
		}
	}
	
	private void hoeherFullHouse(ArrayList<Card> cards1, ArrayList<Card> cards2) {
		int dreier1 = 0;
		int dreier2 = 0;
		int paar1 = 0;
		int paar2 = 0;
		Collections.sort(cards1);
		if(cards1.get(0).getZahl()==cards1.get(1).getZahl() && cards1.get(1).getZahl()==cards1.get(2).getZahl()) {
			if(cards1.get(3).getZahl()==cards1.get(4).getZahl()) {
				dreier1 = cards1.get(0).getZahl();
				paar1 = cards1.get(3).getZahl();
			}
		} else if(cards1.get(0).getZahl()==cards1.get(1).getZahl()) {
			if(cards1.get(2).getZahl()==cards1.get(3).getZahl() && cards1.get(3).getZahl()==cards1.get(4).getZahl()) {
				dreier1 = cards1.get(0).getZahl();
				paar1 = cards1.get(2).getZahl();
			}
		}
		Collections.sort(cards2);
		if(cards2.get(0).getZahl()==cards2.get(1).getZahl() && cards2.get(1).getZahl()==cards2.get(2).getZahl()) {
			if(cards2.get(3).getZahl()==cards2.get(4).getZahl()) {
				dreier2 = cards2.get(0).getZahl();
				paar2 = cards2.get(3).getZahl();
			}
		} else if(cards2.get(0).getZahl()==cards2.get(1).getZahl()) {
			if(cards2.get(2).getZahl()==cards2.get(3).getZahl() && cards2.get(3).getZahl()==cards2.get(4).getZahl()) {
				dreier2 = cards2.get(0).getZahl();
				paar2 = cards2.get(2).getZahl();
			}
		}
		if(dreier1>dreier2) {
			sieg1++;
		} else if(dreier2>dreier1) {
			sieg2++;
		} else if(paar1>paar2) {
			sieg1++;
		} else if(paar2>paar1) {
	...
	private void hoeherFlush(ArrayList<Card> cards1, ArrayList<Card> cards2) {
		Collections.sort(cards1);
		Collections.sort(cards2);
		for(int i=4;i>-1;i--) {
			if(cards1.get(i).getZahl()>cards2.get(i).getZahl()) {
				sieg1++;
				break;
			} else if(cards2.get(i).getZahl()>cards1.get(i).getZahl()) {
				sieg2++;
				break;
			}
		}
	...
	private void hoeherStraight(ArrayList<Card> cards1, ArrayList<Card> cards2) {
		Collections.sort(cards1);
		Collections.sort(cards2);
		if(cards1.get(4).getZahl()>cards2.get(4).getZahl()) {
			sieg1++;
		} else if(cards2.get(4).getZahl()>cards1.get(4).getZahl()) {
			sieg2++;
		}
	}
	
	private void hoeherThreeOfAKind(ArrayList<Card> cards1, ArrayList<Card> cards2) {
		int hoch1 = 0;
		int hoch2 = 0;
		for(int i=0;i<5;i++) {
			int zahlTemp = cards1.get(i).getZahl();
	...
	private void hoeherTwoPairs(ArrayList<Card> cards1, ArrayList<Card> cards2) {
		int paar11 = 0;
		int paar12 = 0;
		int paar21 = 0;
		int paar22 = 0;
		
		ArrayList<Card> cardsTemp1 = new ArrayList<Card>();
		for(int i=0;i<5;i++) {
			cardsTemp1.add(cards1.get(i));
		}
		ArrayList<Card> cardsTemp2 = new ArrayList<Card>();
		for(int i=0;i<5;i++) {
			cardsTemp2.add(cards1.get(i));
		}
		
		n:for(int a=0;a<5;a++) {
			for(int b=0;b<5;b++) {
				if(cards1.get(a).getZahl()==cards1.get(b).getZahl() && a!=b) {
					paar11 = cards1.get(a).getZahl();
					cardsTemp1.remove(a);
					if(a<b) {
						cardsTemp1.remove(b-1);
					} else {
						cardsTemp1.remove(b);
					}
					break n;
		...
		if(cardsTemp1.size()==3) {
			for(int a=0;a<3;a++) {
				for(int b=0;b<3;b++) {
					if(cardsTemp1.get(a).getZahl()==cardsTemp1.get(b).getZahl() && a!=b) {
						paar12 = cardsTemp1.get(a).getZahl();
					}
				}
			}
		}
		...
		n:for(int a=0;a<5;a++) {
			for(int b=0;b<5;b++) {
				if(cards2.get(a).getZahl()==cards2.get(b).getZahl() && a!=b) {
					paar21 = cards2.get(a).getZahl();
					cardsTemp2.remove(a);
					if(a<b) {
						cardsTemp2.remove(b-1);
					} else {
						cardsTemp2.remove(b);
					}
					break n;
		...
		if(cardsTemp2.size()==3) {
			for(int a=0;a<3;a++) {
				for(int b=0;b<3;b++) {
					if(cardsTemp2.get(a).getZahl()==cardsTemp2.get(b).getZahl() && a!=b) {
						paar22 = cardsTemp2.get(a).getZahl();
					}
				}
			}
		}
	...
	private void hoeherOnePair(ArrayList<Card> cards1, ArrayList<Card> cards2) {
		int paar1 = 0;
		int paar2 = 0;
		for(int a=0;a<5;a++) {
			for(int b=0;b<5;b++) {
				if(cards1.get(a).getZahl()==cards1.get(b).getZahl() && a!=b) {
					paar1 = cards1.get(a).getZahl();
				}
			}
		}
		for(int a=0;a<5;a++) {
	...
	private void hoeherHighCard(ArrayList<Card> cards1, ArrayList<Card> cards2) {
		Collections.sort(cards1);
		Collections.sort(cards2);
		for(int i=4;i>-1;i--) {
			if(cards1.get(i).getZahl()>cards2.get(i).getZahl()) {
	...
	public static PokerHands getPh() {
		return ph;
	}
	//Antwort: 376
}// Ergebnis 379 weicht ab!